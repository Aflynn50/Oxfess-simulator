...
app_id = "836623863185924"
app_code = "c4d52c9d60e80243473848718199911b"
page_id = "oxfess"

# input date formatted as YYYY-MM-DD
since_date = "2010-01-01"
until_date = "2017-12-06"

access_token = app_id + "|" + app_code
        ...
        while has_next_page:
            ...
            for status in statuses['data']:
                ...
                if 'reactions' in status:
                    ...
                    status_data = tuple([status_data[1]])
...
with open('oxfess_facebook_statuses.csv', newline='') as csvfile:
	newlist = []
	spamreader = csv.reader(csvfile, delimiter='|')
	for row in spamreader:
		s = [x for x in ((row[0].replace("\n"," ")).split()) if x.isalnum() or ',' in x or '.' in x or "'" in x]
		if s != [] and (s[-1][-1]) != '.':
			s[-1] = s[-1] + '.'
		newlist = newlist + s 


text_model = markovify.Text(functools.reduce(lambda x,y: x + ' ' + y, newlist))
